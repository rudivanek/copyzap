...
**Version:** 8.9.3 (Workflow Word Count Detection Fix)
...
**Word Counter:** A live word count is displayed below this field, showing you exactly how many words you've entered. This helps you gauge the amount of context you're providing to the AI.
...
**Word Counter:** A live word count is displayed below this field, showing you exactly how many words you've entered. This helps you see the length of the copy you're working with.
...
**What it is:** A persistent modal that appears when you enter the Copy Maker, helping you choose the fastest path to start your content creation work. Think of it as a complete launcher that eliminates decision fatigue and routes you directly to the right tool with the right settings.
...
**Start Hub is organized into two main sections:**

---

#### Section 1: Create New Copy

This section contains tools for creating fresh content from scratch.
...
This section provides quick-access tools for improving, polishing, and transforming existing text.

**4. Purpose Rewrite**
Best for: Quickly improving existing copy with purpose-based polishing. A streamlined module designed for fast, focused improvements.

Features:
- Direct navigation to the Purpose Rewrite module
- Minimal setup required
- Intent-based improvement system
- Quick, focused refinements

**5. Copy Snap**
Best for: Quick rewrite, answer, or transform operations with minimal setup. Perfect for fast text transformations.

Features:
- Direct navigation to the Copy Snap module
- Instant text transformation
- Multiple operation modes (rewrite, answer, transform)
...
### 2.17 Purpose Rewrite

**Purpose Rewrite** is a specialized AI-powered tool designed for rapid, purpose-based content refinement. It polishes and optimizes your text based on specific marketing intents (e.g., Social Media Ad, Email Marketing, Landing Page) with full control over audience, goal, tone, and calls to action.

#### Overview

Purpose Rewrite uses preset templates to guide content optimization for specific marketing channels and purposes. Unlike Copy Maker (comprehensive and project-focused) or Copy Snap (quick and lightweight), Purpose Rewrite strikes a balance - providing structured, purpose-driven refinement with optional contextual controls.

**Access:** Purpose Rewrite is accessible via the main navigation or Start Hub.
...
Purpose Rewrite provides specialized presets for common marketing scenarios:
...
Purpose Rewrite now saves complete sessions just like Copy Snap:
...
#### Continue in Copy Maker (Handoff to Full Editor)

After generating polished variants in Purpose Rewrite, you can optionally **continue editing in Copy Maker** for additional refinement, voice styling, scoring, and advanced features.

**How It Works:**

1. **Generate Variants**: Use Purpose Rewrite to create polished outputs
2. **Select One Output**: Click the radio button next to the variant you want to continue with
3. **Continue Button**: Click "Continue in Copy Maker" button (appears after results are generated)
4. **Seamless Handoff**: Copy Maker opens with your selected output pre-filled

**What Gets Transferred:**

When you continue to Copy Maker, the following data is automatically transferred:
- **Selected polished output** - Pre-filled as your working copy
- **Original input text** - Preserved for reference
- **Language** - Maintained from Purpose Rewrite
- **Intent metadata** - Intent ID, label, audience, goal, tone, CTA

**Important Notes:**

- **Selection Required**: You must select exactly ONE output before the "Continue in Copy Maker" button becomes enabled
- **Client-Side Only**: The handoff uses router state with sessionStorage fallback (no database storage)
- **No Auto-Generation**: Copy Maker loads with your content but does not automatically trigger generation, scoring, or voice analysis
- **Full Control**: Once in Copy Maker, you have access to all advanced features:
  - Brand voice application
  - Content scoring and comparison
  - Alternative variants generation
  - SEO optimization
  - Export and template management

**When to Use This Feature:**

- You want to apply brand voices to your polished content
- You need content scoring and evidence-based analysis
- You want to generate alternative variants using different AI models
- You need to create SEO metadata (title, description, schema)
- You want to save the content as a template for reuse

**UX Details:**

- When results are shown, the sticky bottom bar displays two buttons:
  - "Polish Again" (left) - Generate new variants with current settings
  - "Continue in Copy Maker" (right, primary) - Navigate to Copy Maker with selected output
- The "Continue in Copy Maker" button is disabled (grayed out) until you select an output
- Hover tooltip shows: "Select an output first" when disabled
- After navigation, Copy Maker shows a success toast: "Content loaded from Purpose Rewrite"

**Workflow Example:**

1. Use Purpose Rewrite to quickly improve your social media ad copy
2. Get 3 polished variants back
3. Select the variant you like best
4. Click "Continue in Copy Maker"
5. Apply your brand voice in Copy Maker
6. Generate alternative versions using different models
7. Compare and score all versions
8. Export the final winner

This feature bridges the gap between Purpose Rewrite (fast, purpose-focused) and Copy Maker (comprehensive, feature-rich), giving you the best of both workflows.

---

#### Iterative Refinement (Refine This Result)

Purpose Rewrite supports **iterative refinement** through the "Refine this result" post-action, allowing you to polish existing outputs further without starting over.

**How It Works:**

1. **Refine Button**: Each output card has a "Refine" button next to the "Copy" button
2. **Optional Notes**: Click "Refine" to open a modal where you can add refinement notes (optional)
3. **Same Intent & Guardrails**: Refinement uses the selected output as new input while keeping:
   - Same intent (no inference or changes)
   - Same field values (audience, goal, tone, CTA)
   - Same guardrails (language rule, trust rule, soft length awareness)
   - Same content type (plain or HTML)
4. **Refinement Notes**: Combined with existing Special Instructions (if any)
5. **New Card**: Refined result appears as a new card labeled "Refined" with a sparkle icon
6. **Iterative**: You can refine refined results multiple times

**What You Can Refine:**

✅ **Allowed**: Make more concise, emphasize benefits, use simpler words, adjust tone within preset, fix wording, clarify message, adjust length

❌ **NOT Allowed**: Change intent, add new claims (unless intent allows), invent features, switch content type, relax guardrails

**Refinement Notes Examples:**
...
If you have Special Instructions set, refinement notes are combined:
```
Special Instructions (original): <existing instructions>
Refinement Notes: <your refinement notes>
```

**Why Use Refinement:**

1. **Iterative Improvement** - Polish outputs progressively without re-entering data
2. **Keeps Context** - Intent and field values preserved automatically
3. **Fast** - No need to copy/paste or reset form
4. **Guardrails Maintained** - Trust rule and language rule still apply
5. **Multiple Attempts** - Try different refinement approaches on same base text

**Cost:**

- Each refinement = 1 new generation (Claude Sonnet 4.5)
- Same cost as generating 1 variant
- Token usage tracked separately per refinement

**When to Use Refinement:**

- Output is close but needs minor adjustments
- Want to test different tonal variations of same output
- Need to shorten/lengthen without changing meaning
- Want to emphasize different aspects of same message

**Refinement vs. New Generation:**

| Refinement | New Generation |
|-----------|----------------|
| Uses output as new input | Uses original input |
| Keeps all settings | Can change settings |
| Appends new card | Replaces all results |
| 1 variant always | 1-3 variants selectable |
| Best for minor tweaks | Best for major changes |

---

#### Micro-Confirmation (Process Reassurance)

Each Purpose Rewrite output card displays **micro-confirmation tags** - lightweight visual indicators showing what rules and guardrails were applied during generation.

**Important:** These are NOT scores, ratings, or quality judgments. They simply confirm what the system enforced.

**What Micro-Confirmation Shows:**

Micro-confirmation tags are **rule-based** (not AI evaluation) and derived from:
- Selected intent
- Tone setting
- Trust policy mode (strict vs. persuasion allowed)
- Content type (plain vs. HTML)
- Whether the result is a refinement

**Common Tags You'll See:**

**Base Process Tags:**
- **Tone aligned** - Applied your selected tone preset
- **Clarity improved** - Standard polish operation applied
- **Meaning preserved** - Original message maintained
- **Language preserved** - Output in same language as input

**Trust/Guardrail Tags:**
- **No new claims** - Strict trust mode enforced (About, Email Opening, Problem Statement, Section Body)
- **No CTA added** - No call-to-action introduced (strict intents only)
- **No invented claims** - Modest persuasion allowed but no fabrication (other intents)

**Content Type Tags:**
- **HTML preserved** - HTML structure and tags maintained (HTML mode only)

**Intent-Specific Tags** (one per intent):
- **Concise by design** - Hero Headline + Subheadline
- **Improved flow** - Section Body Copy
- **Balanced tone** - About Section
- **Skimmable structure** - Product Description (Short)
- **Action-focused** - Call to Action
- **Search-friendly length** - SEO Snippet
- **Conversational opening** - Email Opening
- **Caption-ready** - Instagram Caption
- **Punchy and compact** - Short Ad Copy
- **Value distilled** - Value Proposition
- **Pain clarified** - Problem / Pain Statement

**Refinement Tag:**
- **Refinement applied** - This result was created via "Refine this result" (always shown first)

**Display Format:**

Tags appear as small blue chips below each output card header:

```
[Refinement applied] [Action-focused] [Tone aligned] [No new claims]
```

**Tag Selection Logic:**

Each card shows 3-4 tags maximum, selected in this priority order:
1. Refinement applied (if refined)
2. Intent-specific tag (1 tag)
3. Tone aligned
4. Trust tag (No new claims / No CTA added / No invented claims)
5. HTML preserved (if HTML mode and room available)

**Why Micro-Confirmation Matters:**

1. **Trust** - See exactly what guardrails were enforced
2. **Transparency** - No hidden AI decisions or evaluations
3. **Consistency** - Same rules apply every time for same intent
4. **Learning** - Understand what each intent prioritizes

**Examples:**

**Hero Headline (Strict Intent):**
```
[Concise by design] [Tone aligned] [No new claims]
```

**Product Description (Persuasion Intent, HTML Mode):**
```
[Skimmable structure] [Tone aligned] [No invented claims] [HTML preserved]
```

**Refined Email Opening:**
```
[Refinement applied] [Conversational opening] [Tone aligned]
```

**What Micro-Confirmation Does NOT Show:**

- ❌ Quality scores or ratings
- ❌ AI evaluation or judgment
- ❌ Better/worse comparisons
- ❌ Predicted performance
- ❌ User preference data

Micro-confirmation is purely **process integrity confirmation** - showing what rules were applied, not evaluating results.
...
#### Soft Length Awareness (Intelligent Output Sizing)

Purpose Rewrite now includes **soft length awareness** - an invisible AI guidance system that ensures outputs naturally feel the right length and density for each intent, without requiring any word count input from the user.

**How It Works:**

Each intent has a built-in "length profile" that guides the AI to produce appropriately sized content:

| Intent | Length Profile | What This Means |
|--------|----------------|-----------------|
| Hero Headline + Subheadline | CONCISE | Prefers brevity, avoids long sentences, keeps impact high without over-explaining |
| Section Body Copy | MODERATE | Allows natural paragraph development with short-to-medium sentences, prioritizes flow and readability |
| About Section | MODERATE | Balanced explanation that's neither terse nor verbose, preserves narrative flow |
| Product Description (Short) | COMPACT | Tight structure with benefit-first phrasing, minimal filler, no long explanations |
| Call to Action | VERY SHORT | Minimal words with strong verbs, no explanation or supporting paragraphs |
| SEO Snippet | SHORT | Concise phrasing with front-loaded clarity, suitable for search preview contexts |
| Email Opening | SHORT TO MODERATE | Conversational opening in one short paragraph, invites reading without overloading |
| Instagram Caption | SHORT | Crisp and engaging, avoids long blocks while allowing line breaks |
| Short Ad Copy | VERY COMPACT | Punchy with minimal words, high signal and low noise |
| Value Proposition | COMPACT | Clear and distilled with one focused idea, no elaboration beyond core value |
| Problem / Pain Statement | SHARP | Direct and focused, makes the pain clear quickly without extended explanation |

**Key Benefits:**

- **No UI clutter** - No length controls, no word count fields to configure
- **Intent-appropriate sizing** - Hero copy stays punchy, body copy stays readable, CTAs stay brief
- **Natural output** - AI shapes length based on content type, not arbitrary numbers
- **Qualitative guidance** - Soft constraints (not hard limits) for natural-feeling results

**How It Interacts with Special Instructions:**

If you provide explicit length instructions in Special Instructions (e.g., "~80 words" or "keep it under 100 words"):
- The AI will respect your explicit instruction
- BUT it will still apply the intent's length *style* (e.g., concise wording vs. flowing sentences)

If you provide conflicting instructions (e.g., "write a long essay" for a CTA intent):
- The AI will ignore the conflicting instruction
- Intent behavior is always preserved

**When It Applies:**

- Always active for all Purpose Rewrite generations
- Works invisibly - no configuration needed
- Applies even when user doesn't specify any word count
- Ensures each intent produces contextually appropriate output length

**Examples:**

✅ **Hero Headline** (CONCISE profile)
Input: "We help businesses grow online with digital marketing services including SEO, content marketing, social media management, and paid advertising campaigns"
Output: "Grow Your Business Online. Full-Service Digital Marketing That Delivers Results."

✅ **Email Opening** (SHORT TO MODERATE profile)
Input: "We wanted to reach out to you today because we have some exciting news about our product launch"
Output: "I have exciting news for you. We just launched a product that solves [specific problem] in a way you've never seen before."

✅ **Call to Action** (VERY SHORT profile)
Input: "If you're interested in learning more about our services and how we can help you achieve your goals, please click here to schedule a consultation with our team"
Output: "Schedule Your Consultation"

---

#### Global Trust Rule (Prevent Over-Marketing Guardrail)

Purpose Rewrite includes a **global trust rule** - an invisible AI guardrail that prevents accidental over-marketing and maintains content integrity across all intents.

**What It Does:**

The trust rule prevents the AI from introducing marketing elements that weren't present in your original text, ensuring outputs stay grounded in your actual content rather than AI-invented claims.

**Core Principle:**

**"Do NOT introduce new marketing claims, promises, guarantees, or calls-to-action unless explicitly allowed by the selected intent."**

**Forbidden Additions (Unless Intent Allows):**

| Category | Examples of What's Blocked |
|----------|---------------------------|
| Marketing Claims | "best", "#1", "guaranteed", "proven", "world-class", "unmatched", "industry-leading" |
| Promises/Guarantees | "will increase revenue", "guarantees results", "instant results" |
| CTAs/Urgency | "Buy now", "Sign up today", "Limited time", "Don't miss out" |
| Invented Benefits | Features or outcomes not present in original input |
| Overhyping Tone | Adding persuasion beyond your original content |

**Always Allowed Improvements:**

Even with strict trust mode, Purpose Rewrite can always improve:
- Clarity, grammar, readability, structure
- Rewording existing claims into clearer language (WITHOUT increasing strength)
- Light tightening of tone consistent with selected Tone
- Better flow and scannability
- Replacing buzzwords with plain language

**Intent-Specific Policy:**

Each intent has a trust policy that controls whether marketing additions are allowed:

**STRICT TRUST MODE** (No marketing additions):
- Section Body Copy (Web / Landing Page)
- About Section
- Email Opening
- SEO Snippet
- Value Proposition
...
In strict trust mode:
- NO CTAs unless explicitly provided in CTA field
- NO new marketing claims or promises
- Focus ONLY on clarity, readability, and grammar

**MODEST PERSUASION ALLOWED** (Limited marketing language):
- Call to Action (CTA intent - strong verbs, very short)
- Short Ad Copy (persuasive but no invented claims)
- Product Description (Short) (benefit-first but grounded)
- Instagram Caption (light promotional, no invented claims)

Even when persuasion is allowed:
- CANNOT invent new benefits or features
- CANNOT create promises/guarantees not in original input
- Must keep persuasion grounded in existing content

**Examples:**

✅ **Allowed - Section Body Copy (Strict Mode)**
Input: "Our software helps teams collaborate better by providing tools for project management and communication."
Output: "Our software streamlines team collaboration through integrated project management and communication tools."
*(Clarified without adding new claims)*

❌ **Blocked - Section Body Copy (Strict Mode)**
Input: "Our software helps teams collaborate better."
Output: "Our industry-leading software guarantees 50% faster collaboration and proven ROI within 30 days."
*(Added marketing claims, promises, and guarantees not in original)*

✅ **Allowed - CTA Intent (Persuasion Allowed)**
Input: "If you want to learn more about our services, please contact us to schedule a meeting with our team."
Output: "Schedule Your Free Consultation"
*(Appropriate CTA - short, verb-driven, based on original intent)*

❌ **Blocked - Problem Statement (Strict Mode)**
Input: "Many businesses struggle with managing their social media presence."
Output: "Tired of wasting hours on social media? Get instant results with our proven platform. Start your free trial today!"
*(Added solution language, invented claims, and CTA - violates problem statement intent)*

**Special Instructions Interaction:**

If you add Special Instructions that request marketing additions:

**Conflicting Requests Ignored:**
- "Add a CTA" when intent is Problem Statement → Ignored
- "Make it sound like the best in the market" when no comparable claim exists → Ignored
- "Add urgency language" when intent is About Section → Ignored

**Respecting User Intent:**
- Explicit length requests → Always respected
- Tone refinement → Always respected
- Formatting constraints → Always respected
- "Keep specific phrases" → Always respected

**Why This Matters:**

1. **Trust in Polish** - Output won't suddenly become more salesy than your input
2. **Intent Integrity** - Problem statements stay problems, not sales pitches
3. **Accuracy** - No invented benefits or promises
4. **Professional Quality** - Output reflects what you actually offer, not AI imagination

**When Trust Rule Applies:**

- Always active for all Purpose Rewrite generations
- Works invisibly - no configuration needed
- Enforced through system prompt rules
...
Purpose Rewrite currently supports **English and Spanish only**.

**How It Works:**

1. **Automatic Detection**: Purpose Rewrite detects whether your input is English or Spanish using pattern-based analysis
2. **Same-Language Output**: Variants are produced in the detected language (English or Spanish)
3. **No Translation**: Content is polished, not translated
...
| English | English |
| Spanish | Spanish |
| Other languages | Not currently supported — defaults to English |

**Language Detection Method:**

Purpose Rewrite uses pattern-based detection (common words and character sets) to identify English or Spanish. Inputs in other languages are not reliably detected and will default to English output.

**Current Limitation:**

Purpose Rewrite does not support languages other than English and Spanish at this time. Additional language support is not implemented.

**When Language Detection Applies:**

- Always active for all Purpose Rewrite generations
- Works invisibly - no language selector needed
- English and Spanish detection enforced through prompt construction

---

#### Tone Options

Purpose Rewrite offers the following tone presets:
...
Purpose Rewrite uses Claude Sonnet 4.5, providing high-quality, purpose-optimized refinement:
...
**Token Tracking:**
- All Purpose Rewrite generations tracked under operation type: `quick-polish`
...
- Premium quality-to-cost ratio for purpose-driven work

---

#### Best Practices

**When to use Purpose Rewrite:**
...
- Use "Refine this result" for iterative improvement instead of starting over
- Test different tones to find what resonates
- Refine refined results multiple times for progressive polish
...
- Excellent at purpose-based optimization
...
**Changes:**
- **NEW:** Purpose Rewrite now includes **"Continue in Copy Maker" handoff** - after generating polished variants, you can select one output and click "Continue in Copy Maker" to seamlessly transfer it to Copy Maker for advanced editing. Features: (1) Radio button selection per variant, (2) Button disabled until one output selected, (3) Transfers selected output + original input + intent metadata via navigation state (client-side only, no database), (4) Copy Maker pre-fills the content without auto-triggering generation/scoring, (5) Enables full Copy Maker features: brand voices, scoring, alternatives, SEO, templates. Perfect workflow: Purpose Rewrite for fast improvements → Copy Maker for comprehensive refinement.
- **NEW:** Purpose Rewrite now includes **micro-confirmation tags** - each output card displays 3-4 small visual tags showing what rules and guardrails were applied (e.g., "Tone aligned", "No new claims", "HTML preserved", "Refinement applied"). Rule-based only (NOT AI evaluation or scoring). Shows process integrity: intent-specific policies, trust mode enforcement, content type handling. Purely reassurance display - no quality judgment or ratings. Tags derived from intent + tone + trust mode + content type + refinement status. Max 4 tags per card in blue chip format. Zero extra API calls - pure rule-based display logic.
- **NEW:** Purpose Rewrite now includes **iterative refinement (Refine this result)** - each output card has a "Refine" button that re-runs Purpose Rewrite using that output as new input while preserving intent, guardrails, and field values. Add optional refinement notes to guide adjustments. Refined results append as new cards labeled "Refined" with sparkle icon. Supports multiple refinement iterations. Uses same cost as 1 variant generation (Claude Sonnet 4.5). No Copy Maker features - strictly constrained to polish refinement within same intent.
- **NOTE:** Purpose Rewrite includes automatic language detection for English and Spanish. The system detects the input language and produces output in the same language. English and Spanish are the only supported languages. Inputs in other languages are not reliably detected and default to English output.
- **NEW:** Purpose Rewrite now includes **global trust rule (prevent over-marketing guardrail)** - invisible AI guardrail that prevents introducing new marketing claims, promises, or CTAs unless explicitly allowed by the intent. Maintains content integrity by keeping outputs grounded in your actual content. Intent-specific policies (strict trust mode vs. modest persuasion allowed). Works with Special Instructions conflict resolution.
- **NEW:** Purpose Rewrite now includes **soft length awareness** - invisible AI guidance that ensures outputs naturally feel the right length for each intent (Hero = concise, CTA = very short, Body = moderate, etc.) without any UI controls or word count inputs
- Purpose Rewrite now uses **complete session saving** - saves input + all variants together in one record, matching Copy Snap's save behavior
- Purpose Rewrite sessions now appear in Dashboard > Saved Outputs (NOT in Copy Sessions or Credits Usage tabs)
- Opening saved Purpose Rewrite sessions from Dashboard correctly routes to Purpose Rewrite (not Copy Maker)
- Complete session restoration - all input settings and variants reload perfectly
- Purpose Rewrite usage tracking is separate from Copy Maker/Copy Snap - does not appear in Credits Usage tab
...
- **Word count is automatically detected** from the content being processed and applied to all workflow steps
...
Workflows automatically respect the word count of the content being processed:
- When transferring a variant (e.g., 26 words) to Copy Maker and running a workflow → all outputs maintain 26 words
- When using "Improve existing copy" mode with content (e.g., 21 words) and running a workflow → all outputs maintain 21 words
- Word count is detected from the source content, not from form settings
- This ensures consistency across all workflow-generated alternatives
